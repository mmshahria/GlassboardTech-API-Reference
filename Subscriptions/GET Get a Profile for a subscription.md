Retrieves the details of an existing profile for a subscription. Supply the unique `id` either from a profile creation request or the profile list to retrieve the specified profile's information for a subscription.
