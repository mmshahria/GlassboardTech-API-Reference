Retrieves the details of an existing profile for a valid identifier. Supply the unique `profileId` either from a profile creation request or the profile list to retrieve specified profile's information.
