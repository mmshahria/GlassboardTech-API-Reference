To create an asset a user must create a deal as a prerequisite. Supply the `key:value` pair that is necessary for describing an asset. Data in the request body should follow the JSON data formatting.


> Asset,  as the context requires, is an investment vehicle that is associated with a deal and can be of many type such as shares of common stock in a company, real estate, cryptocurrency, or even fractional ownership in a private jet. 
